	...
	implements IWorkbenchLayoutService
{
	...
	/** Panel position saved before entering free mode, restored on exit. */
	private panelDialogSavedPosition: Position | undefined;
	/** Panel alignment saved before entering free mode, restored on exit. */
	private panelDialogSavedAlignment: PanelAlignment | undefined;
	/** Panel view size saved before entering free mode, restored on exit. */
	...
	layout(): void {
		if (!this.disposed) {
			...
			// Re-layout the floating panel dialog if it is open in free mode.
			// Only clamp — do not re-scale the dialog; it keeps its pixel size.
			if (this.isVisible(Parts.PANEL_PART) && this.isPanelInFreeMode()) {
	...
	setPanelAlignment(alignment: PanelAlignment): void {
		// In floating dialog mode, alignment changes would corrupt the grid.
		// Stash the new value so it is restored when exiting dialog mode.
		if (this.isPanelInFreeMode()) {
			this.panelDialogSavedAlignment = alignment;
			return;
		}
	...
	private setPanelHidden(hidden: boolean, skipLayout?: boolean): void {
		...
		// Adjust CSS and arrange the panel element based on the active mode.
		const panelPart = this.getPart(Parts.PANEL_PART);
		if (this.isPanelInFreeMode()) {
			// Free (floating dialog) mode: panel lives in the overlay div.
			if (hidden) {
				...
				this.mainContainer.classList.remove(
					LayoutClasses.PANEL_DIALOG_MAXIMIZED,
				);
			} else {
			...
			if (!hidden && this.panelDialogOverlay) {
				...
				if (
					panelPart.element.parentElement !== this.panelDialogOriginalParent
				) {
					this.panelDialogOriginalParent.appendChild(panelPart.element);
				}
			}
		} else {
			// Pinned (grid-docked) mode: panel lives in the workbench grid.
			if (hidden) {
				this.mainContainer.classList.add(LayoutClasses.PANEL_HIDDEN);
		...
		// Notify the panel part of its new visibility state.
		...
		if (hidden) {
			// Remember whether the panel was maximized for next open
			this.stateModel.setRuntimeValue(
		...
		if (!hidden && !skipLayout) {
			if (this.isPanelInFreeMode()) {
				...
				// In pinned mode, honour the "opens maximized" setting.
	...
	private isPanelDialogMaximized(): boolean {
		return this.mainContainer.classList.contains(
			LayoutClasses.PANEL_DIALOG_MAXIMIZED,
		);
	}

	isPanelInFreeMode(): boolean {
		return this.stateModel.getRuntimeValue(
			LayoutStateKeys.PANEL_DIALOG_FREE_MODE,
		);
	}

	togglePanelDialogMode(): void {
		this.setPanelDialogFreeMode(!this.isPanelInFreeMode());
	}

	private setPanelDialogFreeMode(free: boolean): void {
		if (free === this.isPanelInFreeMode()) {
			return;
		}
		const panelPart = this.getPart(Parts.PANEL_PART);
		const panelVisible = this.isVisible(Parts.PANEL_PART);
		if (free) {
			// Save the current panel position and alignment so we can restore them when exiting.
			this.panelDialogSavedPosition = this.getPanelPosition();
			this.panelDialogSavedAlignment = this.getPanelAlignment();
			// If the panel is currently maximized (editor hidden), un-maximize first
			// so the editor is visible behind the floating dialog.
			// This must happen *before* setting the free-mode flag so that
			// isPanelMaximized() / toggleMaximizedPanel() take the grid code path.
			if (panelVisible && this.isPanelMaximized()) {
				this.toggleMaximizedPanel();
			}
			// Now commit the mode change.
			this.stateModel.setRuntimeValue(
				LayoutStateKeys.PANEL_DIALOG_FREE_MODE,
				free,
			);
			// Capture the panel's current grid parent *before* moving it out.
			// The grid slot changes when the panel position changes, so we must
			// refresh this every time we enter free mode.
			this.panelDialogOriginalParent =
				panelPart.element.parentElement ?? undefined;
			// Pinned → Dialog: hide panel in the grid and move it into the overlay.
			// We do NOT change the panel position — the grid preserves the view's
			// slot across a hide/show cycle as long as the element is put back in
			// the same parent (panelDialogOriginalParent).
			// Save the panel's grid size *before* hiding. The splitview caches this
			// internally, but for LEFT/RIGHT positions (horizontal splitview) the
			// internal cache sometimes fails to restore correctly. An explicit save
			// and restore ensures the panel regains its proper space.
			if (panelVisible) {
				this.panelDialogSavedSize = this.workbenchGrid.getViewSize(
					this.panelPartView,
				);
				this.workbenchGrid.setViewVisible(this.panelPartView, false);
			}
			...
			// Commit the mode change first so setPanelPosition takes the grid path.
			this.stateModel.setRuntimeValue(
				LayoutStateKeys.PANEL_DIALOG_FREE_MODE,
				free,
			);
			// Dialog → Pinned: remove panel from the overlay and restore it into the grid.
			this.mainContainer.classList.remove(LayoutClasses.PANEL_DIALOG_OPEN);
			this.mainContainer.classList.remove(LayoutClasses.PANEL_DIALOG_MAXIMIZED);
			// Always restore the panel element to the grid slot regardless of visibility,
			// so the grid can manage it correctly when the panel is next shown.
			if (
				this.panelDialogOriginalParent &&
				panelPart.element.parentElement !== this.panelDialogOriginalParent
			) {
			...
			if (panelVisible) {
				this.workbenchGrid.setViewVisible(this.panelPartView, true);
				// Explicitly restore the panel size. For LEFT/RIGHT positions the
				// splitview's internal cache may not restore properly, so we force
				// the view back to the size it had before entering dialog mode.
				if (this.panelDialogSavedSize) {
					this.workbenchGrid.resizeView(
						this.panelPartView,
						this.panelDialogSavedSize,
					);
					this.panelDialogSavedSize = undefined;
				}
			}
			// Restore the panel position if it was changed while in free mode
			// (e.g. via a keybinding that bypassed our guard).
			if (
			...
			this.panelDialogSavedPosition = undefined;
			// Restore the panel alignment that was active before entering free mode.
			// Only apply alignment if the panel is in a horizontal position (TOP/BOTTOM),
			// because setPanelAlignment forcibly moves the panel to BOTTOM if it's at LEFT/RIGHT.
			if (
	...
	private ensurePanelDialogOverlay(): void {
		...
		// Capture the panel's original grid parent the first time the overlay is
		// created. This is the `.split-view-view` slot the grid manages, and it
		// must be captured while the element is still inside the grid.
		...
		// Safe-area wrapper that clips the floating dialog to the visible editor
		// zone (below the title bar, above the status bar).  overflow: hidden is
		// what prevents the CSS `resize` handle from pulling content outside the
		// workbench chrome.  pointer-events: none passes through clicks to the
		// workbench; the overlay re-enables pointer events for itself.
		...
		// The floating dialog container (appended first so it precedes the backdrop
		// in DOM order — this allows the :focus-within ~ sibling selector to work).
		...
		// Semi-transparent backdrop that dims the workbench only while the panel
		// has focus. It is intentionally not interactive (pointer-events: none)
		// so clicking outside the panel does not close it.
		const backdrop = document.createElement("div");
		backdrop.classList.add("panel-dialog-backdrop");
		safeArea.appendChild(backdrop);

		// Position the safe area to exclude the title bar and status bar.
		this.updatePanelDialogSafeArea();

		// Drag support: drag the overlay by its title bar.
		let dragStartX = 0;
		let dragStartY = 0;
		...
		const onMouseMove = (e: MouseEvent): void => {
			const safeWidth = safeArea.offsetWidth;
			const safeHeight = safeArea.offsetHeight;
			const maxLeft = Math.max(0, safeWidth - overlay.offsetWidth);
			const maxTop = Math.max(0, safeHeight - overlay.offsetHeight);
			const newLeft = Math.max(
				0,
				Math.min(overlayStartLeft + e.clientX - dragStartX, maxLeft),
			);
			const newTop = Math.max(
				0,
				Math.min(overlayStartTop + e.clientY - dragStartY, maxTop),
			);
		...
		overlay.addEventListener("mousedown", (e: MouseEvent) => {
			...
			if (!titleEl || !titleEl.contains(target)) {
				return;
			}
			if (
				target.closest(
					"a, button, .action-item, .action-label, .composite-bar, input, select",
				)
			) {
				return;
			}
		...
		// Re-layout the panel content whenever the overlay is resized via the CSS resize handle.
	...
	/**
	 * Positions the safe-area div to span between the title bar and the status
	 * bar, so the floating dialog is always confined to the editor zone.
	 */
	private updatePanelDialogSafeArea(): void {
		if (!this.panelDialogSafeArea) {
			return;
		}
		const titleBarH = this.isVisible(Parts.TITLEBAR_PART)
			? this.titleBarPartView.minimumHeight
			: 0;
		const statusBarH = this.isVisible(Parts.STATUSBAR_PART)
			? this.statusBarPartView.minimumHeight
			: 0;
	...
	private layoutPanelDialog(): void {
		...
		// Restore persisted bounds from a previous session, or compute a sensible default.
		if (!this.panelDialogOverlay.style.width) {
			...
			if (saved) {
				...
				// Default position: 70% wide, 40% tall, near the bottom of the safe area.
				const titleBarH = this.isVisible(Parts.TITLEBAR_PART)
					? this.titleBarPartView.minimumHeight
					: 0;
				const statusBarH = this.isVisible(Parts.STATUSBAR_PART)
					? this.statusBarPartView.minimumHeight
					: 0;
				const safeWidth = this._mainContainerDimension.width;
				const safeHeight = Math.max(
					0,
					this._mainContainerDimension.height - titleBarH - statusBarH,
				);
				const dialogWidth = Math.round(safeWidth * 0.7);
				const dialogHeight = Math.round(safeHeight * 0.4);
				const left = Math.round((safeWidth - dialogWidth) / 2);
				const top = safeHeight - dialogHeight - 8;

				this.panelDialogOverlay.style.width = `${dialogWidth}px`;
				this.panelDialogOverlay.style.height = `${dialogHeight}px`;
				this.panelDialogOverlay.style.left = `${left}px`;
				this.panelDialogOverlay.style.top = `${top}px`;
				// Persist ratios immediately so window-resize stays proportional.
				if (safeWidth > 0 && safeHeight > 0) {
					this.storageService.store(
						"workbench.panel.dialogBoundsRatio",
						JSON.stringify({
							leftRatio: left / safeWidth,
							topRatio: top / safeHeight,
							widthRatio: dialogWidth / safeWidth,
							heightRatio: dialogHeight / safeHeight,
						}),
	...
	private loadPanelDialogBounds():
		...
		if (raw) {
			try {
				const { leftRatio, topRatio, widthRatio, heightRatio } =
					JSON.parse(raw);
				// Validate that all values are numeric ratios in a sensible range.
				if (
					...
					// Ratios are stored relative to the safe-area dimensions.
					const titleBarH = this.isVisible(Parts.TITLEBAR_PART)
						? this.titleBarPartView.minimumHeight
						: 0;
					const statusBarH = this.isVisible(Parts.STATUSBAR_PART)
						? this.statusBarPartView.minimumHeight
						: 0;
					const cw = this._mainContainerDimension.width;
					const ch = Math.max(
						0,
						this._mainContainerDimension.height - titleBarH - statusBarH,
					);
					const width = Math.round(widthRatio * cw);
					const height = Math.round(heightRatio * ch);
					// Clamp position so the dialog starts fully inside the safe area.
					const left = Math.max(
						0,
						Math.min(Math.round(leftRatio * cw), cw - width),
					);
					const top = Math.max(
						0,
						Math.min(Math.round(topRatio * ch), ch - height),
					);
	...
	private savePanelDialogBounds(): void {
		...
		// Store ratios relative to the safe-area dimensions so that positions
		// restore proportionally when the window is resized between sessions.
		const containerWidth = this.panelDialogSafeArea.offsetWidth;
		const containerHeight = this.panelDialogSafeArea.offsetHeight;
		...
		this.storageService.store(
			"workbench.panel.dialogBoundsRatio",
			JSON.stringify({
				leftRatio: this.panelDialogOverlay.offsetLeft / containerWidth,
				topRatio: this.panelDialogOverlay.offsetTop / containerHeight,
				widthRatio: this.panelDialogOverlay.offsetWidth / containerWidth,
				heightRatio: this.panelDialogOverlay.offsetHeight / containerHeight,
			}),
			StorageScope.PROFILE,
			StorageTarget.MACHINE,
		);
	...
	private layoutPanelFromOverlay(): void {
		if (!this.panelDialogOverlay || !this.isVisible(Parts.PANEL_PART)) {
			return;
		}
		// Keep the safe-area insets up to date so clamping uses the correct bounds.
		this.updatePanelDialogSafeArea();
	...
	private clampPanelDialogOverlay(): void {
		...
		const safeWidth = this.panelDialogSafeArea.offsetWidth;
		const safeHeight = this.panelDialogSafeArea.offsetHeight;

		// Clamp size so the overlay never extends beyond the safe area.
		// This covers the case where the CSS resize handle is dragged outside.
		if (this.panelDialogOverlay.offsetWidth > safeWidth) {
			this.panelDialogOverlay.style.width = `${safeWidth}px`;
		}
		if (this.panelDialogOverlay.offsetHeight > safeHeight) {
			this.panelDialogOverlay.style.height = `${safeHeight}px`;
		}

		const overlayWidth = this.panelDialogOverlay.offsetWidth;
		const overlayHeight = this.panelDialogOverlay.offsetHeight;
		const maxLeft = Math.max(0, safeWidth - overlayWidth);
		const maxTop = Math.max(0, safeHeight - overlayHeight);
		const clampedLeft = Math.max(
			0,
			Math.min(this.panelDialogOverlay.offsetLeft, maxLeft),
		);
		const clampedTop = Math.max(
			0,
			Math.min(this.panelDialogOverlay.offsetTop, maxTop),
		);
	...
	isPanelMaximized(): boolean {
		// In floating dialog mode, check whether the dialog itself is maximized
		if (this.isPanelInFreeMode()) {
	...
	toggleMaximizedPanel(): void {
		// In floating dialog mode, toggle the dialog between its default size and full workbench size
		if (this.isPanelInFreeMode()) {
	...
	setPanelPosition(position: Position): void {
		// In floating dialog mode, position changes would corrupt the grid.
		// Stash the new value so it is restored when exiting dialog mode.
		if (this.isPanelInFreeMode()) {
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const panelNode: ISerializedLeafNode = {
			...
			size: panelSize,
			// In free (dialog) mode the panel always floats in the overlay; in pinned mode use normal visibility.
			visible:
				!this.isPanelInFreeMode() &&
				!this.stateModel.getRuntimeValue(LayoutStateKeys.PANEL_HIDDEN),
		};
...
const LayoutStateKeys = {
	...
	PANEL_DIALOG_FREE_MODE: new RuntimeStateKey<boolean>(
		"panel.dialogFreeMode",
		StorageScope.PROFILE,
		StorageTarget.USER,
		true,
	),
