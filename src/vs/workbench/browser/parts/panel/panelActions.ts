...
const panelFloatIcon = registerIcon(
	"panel-float",
	Codicon.pin,
	localize("panelFloatIcon", "Icon to float the panel as a dialog."),
);
const panelDockIcon = registerIcon(
	"panel-dock",
	Codicon.pinned,
	localize("panelDockIcon", "Icon to dock the panel to the workbench."),
);
...
registerAction2(
	class extends Action2 {
		constructor() {
			super({
				id: "workbench.action.togglePanelDialogMode",
				title: localize2("togglePanelDialogMode", "Toggle Panel Pin Mode"),
				tooltip: localize("pinPanel", "Pin Panel to Workbench"),
				category: Categories.View,
				f1: true,
				icon: panelFloatIcon,
				toggled: {
					condition: PanelDialogFreeModeContext.negate(),
					icon: panelDockIcon,
					tooltip: localize("unpinPanel", "Unpin Panel from Workbench"),
				},
				menu: [
					{
		...
		run(accessor: ServicesAccessor) {
			accessor.get(IWorkbenchLayoutService).togglePanelDialogMode();
		}
	},
);
...
MenuRegistry.appendMenuItem(MenuId.MenubarAppearanceMenu, {
	...
	order: 4,
	when: PanelDialogFreeModeContext.negate(),
});
